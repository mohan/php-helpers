...
function render_markdown($text, $attrs=[], $enable_shortcodes=false)
{
    ...
    foreach ($lines as $line) {
        ...
        if(preg_match("/^(\t*)```([[:alnum:]\s-]*)$/", $line, $matches)){
            // If close of block
            if($is_block){
                if(_strstr($block_attr, 'table')){
                    $out .= tag_table($data_table_header, $data_table, [], false, false);
                    $data_table_header = [];
                    $data_table = [];
                    $data_table_i = 0;
                } else if(_strstr($block_attr, 'textarea')){
        ...
        if($is_block){
            if(_strstr($block_attr, 'table')) {
                if($data_table_i == 0 && _strstr($block_attr, 'with-header')){
                ...
                $data_table_i++;
                continue;
            } else if(_strstr($block_attr, 'textarea', 'html')) {
                $block_data .= "$line\n";
                continue;
            }
